...
- 현재 서버는 PyPI와 npm에 등록된 패키지 이름·버전·의존성 같은 메타데이터만 판정 근거로 사용합니다. 사람이 공식 문서와 대조해 추가하는 근거 자료가 비어 있는 이유는 [제한사항](#제한사항)에 설명되어 있습니다.

---

## 설치

**요구 사항** — 서버에 연결하는 MCP 클라이언트는 MCP `2026-07-28` 개정판을 지원해야 합니다. 이 서버는 MCP `2026-07-28`만 구현하며, 이전 개정판으로 자동 전환하지 않습니다. 서버를 직접 실행하려면 Python 3.14 이상이 필요합니다. 직접 실행한 서버는 패키지 정보를 가져오기 위해 `pypi.org`와 `registry.npmjs.org`에 HTTPS 요청을 보낼 수 있어야 합니다.
...
- **`get_compatibility_context`** — 릴리스 하나를 받아, 해당 릴리스가 선언한 버전 제약과 호환성을 깨뜨릴 수 있는 변경 사항을 돌려줍니다. 이 도구는 다른 릴리스와 비교하거나 호환 여부를 판정하지 않습니다.
...
| **B** | npm 게시자가 적은 `engines.node`, 사람이 공식 문서와 대조한 지원 정책·릴리스 노트                   | 게시자가 밝힌 지원 또는 비지원 범위로 사용               |
| **C** | PyPI 페이지에서 지원 Python 버전을 표시하는 `Programming Language :: Python` 분류 항목(classifier)  | 다른 근거를 보강할 수 있지만 단독 판정에는 사용하지 않음 |

- **A 등급의 필수 설치 조건을 위반하면 항상 `unsupported`입니다.** 공식 문서에 반대 내용이 있어도 필수 설치 조건을 우선하며, 서로 다른 내용이 발견됐다는 사실은 응답의 `notices` 필드에 남깁니다.
...
**1. 사람이 검토한 공식 문서 근거가 아직 없습니다.** 서버는 패키지 레지스트리의 메타데이터 외에 **큐레이션 팩**이라는 두 번째 근거 모음을 사용할 수 있습니다. 큐레이션 팩은 사람이 공식 문서를 직접 읽고 확인한 지원 범위와 변경 사항을 출처 URL과 함께 기록하는 JSON 파일입니다.

서버에는 큐레이션 팩을 읽고 검증하는 코드가 있지만, 배포되는 큐레이션 팩의 `entries` 목록은 현재 비어 있습니다. 프로젝트는 사람이 공식 문서와 대조하지 않은 내용을 큐레이션 팩에 넣지 않습니다. `get_compatibility_context` 응답은 현재 레지스트리 메타데이터만 사용했다는 뜻의 `depth: registry_only`를 표시합니다. 지원되는 관계를 처리한 `check_compatibility` 응답은 큐레이션 근거가 없다는 뜻의 `curated_pack_missing` 제한 사항을 표시합니다. 지원하지 않는 관계는 큐레이션 팩을 조회하기 전에 끝나므로 `curated_pack_missing` 제한 사항을 표시하지 않습니다. 이러한 표시는 오류가 아니라 현재 사용할 수 있는 근거의 범위를 나타냅니다. Python과 Node.js의 릴리스 날짜 및 지원 종료일 표는 큐레이션 팩과 별도로 공식 출처에서 생성하며, 현재 완전한 자료가 포함되어 있습니다.

**2. 공식 문서를 바탕으로 한 답변 범위는 큐레이션 팩에 기록된 대상과 버전 범위에 달려 있습니다.** 공식 문서로 확인한 지원 선언, 호환성을 깨뜨리는 변경(`breaking_change`), 지원 중단 예고(`deprecation`)는 큐레이션 팩에서만 나옵니다. 질문과 관련된 큐레이션 항목이 없으면 서버는 레지스트리 메타데이터를 정리해 전달하는 데 그칩니다. **요청 처리 과정에서 언어 모델을 사용하지 않기 때문에 답변 범위는 좁지만, 사용자는 모든 판단 근거를 직접 검증할 수 있습니다.**
...
가장 도움이 되는 기여는 **공식 문서를 검토해 큐레이션 팩에 근거를 추가하는 일**입니다. 큐레이션 항목에는 공식 문서에서 직접 확인한 내용, 문서 URL, 검토한 사람, 검토 날짜를 함께 기록해야 합니다. 큐레이션 항목이 늘어날수록 서버가 공식 문서를 근거로 답할 수 있는 질문도 늘어납니다. 작성 형식과 검토 규칙은 [큐레이션 팩 작성 안내](src/dependency_compat_mcp/curated/pack/README.md)에 설명되어 있습니다.
