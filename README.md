...
같은 `MCPServer`를 로컬용 `stdio`와 원격용 무상태 Streamable HTTP로 모두 제공합니다. MCP `2026-07-28`에서는 요청마다 프로토콜 버전과 클라이언트 정보가 전달되므로 연결이나 세션에 판정 상태를 저장하지 않습니다. 이 서버는 이 개정판 **하나만** 구현하며, 이전 개정판 요청은 `-32022`로 거절합니다.

SDK 기본값을 그대로 두지 않고 손댄 곳이 네 군데 있으며, 모두 "광고한 것과 실제 동작이 같아야 한다"는 한 가지 이유에서 나왔습니다.

| 조정                              | 이유                                                                                                                                                                                                                                       |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`2026-07-28`만 서빙**           | SDK는 이전 핸드셰이크 시대를 두 전송 모두에서 계속 받아 주며, 인식하지 못한 버전은 오류 대신 이전 개정판으로 되돌려 협상합니다. 미들웨어가 이를 `-32022`로 거절해 `server/discover`가 광고하는 목록과 실제로 응답하는 범위를 일치시킵니다. |
